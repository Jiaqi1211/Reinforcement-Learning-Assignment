...
def value_policy(policy):
    ...
    value = np.linalg.solve(np.identity(n_states)-gamma*P, r) 
    return value
...
def bruteforce_policies():
    ...
    for i in range(n_actions**len(iterations)):
        value = value_policy(policy)
        if value[0] > optimalvalue[0]:
            optimalvalue = value
            optimalpolicies = [np.copy(policy)]
        elif value[0] == optimalvalue[0]:
            optimalvalue = value
            optimalpolicies.append(np.copy(policy))
            
        policy[0] += 1
        for j in range(len(iterations)-1):
            if policy[iterations[j]] == n_actions:
                policy[iterations[j]] = 0
                policy[iterations[j+1]] += 1    
